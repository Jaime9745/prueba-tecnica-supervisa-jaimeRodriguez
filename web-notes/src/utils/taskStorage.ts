...
export class TaskStorage {
  ...
  static async saveTask(taskData: CreateTaskData): Promise<Task> {
    ...
    const existingTask = tasks.find(task => task.title.toLowerCase() === taskData.title.toLowerCase());
    if (existingTask) {
  ...
  static async updateTask(id: string, updateData: Partial<CreateTaskData>): Promise<Task | null> {
    const tasks = await this.getAllTasks();
    ...
    if (updateData.title) {
      const existingTask = tasks.find(task => 
        task.title.toLowerCase() === updateData.title!.toLowerCase() && task.id !== id
      );
      if (existingTask) {
        throw new Error('A task with this title already exists');
      }
